...
//------- Begin of function FileSystem::get_file_name ---------//
//
...
const char* FileSystem::get_file_name(const char* srcFileName)
{
	...
	{
		if( srcFileName[i]==PATH_DELIM[0] )			// get last delimiter before the file name
			break;
	}

	return srcFileName+i+1;
}
//---------- End of function FileSystem::get_file_name ---------//
