...
void _stdcall IMGremapHMirror(char* imageBuf, int pitch, int x, int y, char* bitmapPtr, unsigned char** colorTableArray)
{
	...
	int srcline_end = 4 + width - 1;		// skip 4 byte header, points to last pixel on a line
	int al;

	for ( int j=0; j<height; ++j, destline+=pitch, srcline_end+=width )
	{
		for ( int i=0; i<width; ++i )
		{
			al = bitmapPtr[ srcline_end - i ];		// NOTE: must be signed
