...
% Stylistic:
	...
	% Some use (in commands sections) of spacing with "/".

% Formatting:
	% Condenced some sentences into 3 sentence minimum parapgraphs -- to correct spacing issues.
	% New manual is a bit longer. Circa 10 pages. Due to sectioning?
	...
	% Sectioning spacing is different.
	% Large, centered graphics standaridized on .9 width to match .5cm indentation. Tiles standardized on 0.1 width.
...
% Content:
	...
	% Credited other manual authors: the3dfxdude, microvirus, jlac.
...
% -- Notes and TODO -- %
	...
	% CODE CLEAN UP.
	% Manually code the tilepage and remove titlepic package. - Graphic on top + Ancient Adversaries... etc.
	% Some tile graphics extend past top textline.
	% Check chapter references.
	% Organize key/mouse commands alphabetically or functionally.
	% Text in wrapfig env.: botttom text has less space than to the left.
	% Uses in, cm, pt, etc. Use just one type?
	% Large "A" graphics on own page centered left - right and top - bottom?
...
\usepackage{graphicx}
\usepackage{titlepic} % Does not place pic on top.
